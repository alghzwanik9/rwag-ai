...
import type { WindowWall, WindowSize, CurtainStyle } from "@/lib/useSceneStore";
import { IKEA_TEMPLATES } from "./config/catalogTemplates";
import { PREDEFINED_TEMPLATES, TemplateType } from "@/lib/templates";
    ...
    scene.traverse((obj: THREE.Object3D) => {
      ...
      if (child.isMesh && child.material) {
        ...
        if (matName === selectedObjectId) {
          if ((material as any).emissive) (material as any).emissive.setHex(0x333333);
        } else {
          if ((material as any).emissive) (material as any).emissive.setHex(0x000000);
        }
...
export default function Page() {
  ...
  const handleLoadTemplate = (type: TemplateType) => {
    ...
    if (type === "LivingRoom") {
      ...
      if (sofa) itemsToAdd.push({ ...sofa, position: [0, 0, -1.5], rotation: [0, 0, 0] });
      
      const table = findAsset(["table", "coffee", "lack"], "tables");
      if (table) itemsToAdd.push({ ...table, position: [0, 0, 0.2], rotation: [0, 0, 0] });
      
      const rug = findAsset(["rug", "carpet"], "rugs");
      if (rug) itemsToAdd.push({ ...rug, position: [0, 0.02, -0.5], rotation: [0, 0, 0] });

      const shelf = findAsset(["shelf", "display", "cabinet"], "storage");
      if (shelf) itemsToAdd.push({ ...shelf, position: [-2.5, 0, 0.5], rotation: [0, Math.PI / 2, 0] });

      const plant = findAsset(["plant", "tree"]);
      if (plant) itemsToAdd.push({ ...plant, position: [-2.5, 0, -1.5], rotation: [0, 0, 0] });

      const decor = findAsset(["vase", "camera", "decor"], "decor");
      if (decor) itemsToAdd.push({ ...decor, position: [1.5, 0, 0.2], rotation: [0, 0, 0] });
    } else if (type === "Bedroom") {
      const chair = findAsset(["chair", "armchair", "wood"], "seating");
      if (chair) itemsToAdd.push({ ...chair, position: [1.5, 0, -1.5], rotation: [0, -Math.PI / 4, 0] });
      
      const cabinet = findAsset(["drawer", "cabinet"], "storage");
      if (cabinet) itemsToAdd.push({ ...cabinet, position: [-1.8, 0, 0.5], rotation: [0, Math.PI / 3, 0] });
      
      const rug = findAsset(["rug", "carpet"], "rugs");
      if (rug) itemsToAdd.push({ ...rug, position: [0, 0.02, 0], rotation: [0, 0, 0] });
      
      const decor = findAsset(["vase", "flower", "camera"], "decor");
      if (decor) itemsToAdd.push({ ...decor, position: [0.5, 0, -1.2], rotation: [0, 0, 0] });
    } else if (type === "Kitchen") {
      const table = findAsset(["outdoor", "table", "dining"], "tables");
      if (table) itemsToAdd.push({ ...table, position: [0, 0, 0], rotation: [0, 0, 0] });
      
      const chair1 = findAsset(["dining", "deck", "chair"], "seating");
      if (chair1) itemsToAdd.push({ ...chair1, position: [-1.2, 0, 0.2], rotation: [0, Math.PI / 2, 0] });
      
      const chair2 = findAsset(["dining", "deck", "chair"], "seating");
      if (chair2) itemsToAdd.push({ ...chair2, position: [1.2, 0, -0.2], rotation: [0, -Math.PI / 2, 0] });

      const plant = findAsset(["plant", "tree"]);
      if (plant) itemsToAdd.push({ ...plant, position: [2, 0, 1.2], rotation: [0, 0, 0] });
    }
    
    if (itemsToAdd.length === 0) {
      // Fallback: use hardcoded PREDEFINED_TEMPLATES (works without API/Agent)
      const fallback = PREDEFINED_TEMPLATES[type];
      if (fallback && fallback.length > 0) {
        itemsToAdd = fallback.map(item => ({ ...item }));
        showToast(`✅ تم تحميل قالب ${type} من الملفات المحلية`);
      } else {
    ...
    itemsToAdd.forEach(item => {
      addSceneItem({ ...item, instanceId: crypto.randomUUID() });
    });
    ...
    <>
      ...
      />
        ...
        <div className={`fixed inset-0 z-[90] pointer-events-none transition-opacity duration-700 ${isTransitioning ? "opacity-100" : "opacity-0"}`}>
          <div className="absolute inset-0 bg-gradient-to-b from-primary/40 via-surface/60 to-surface/80 backdrop-blur-sm" />
